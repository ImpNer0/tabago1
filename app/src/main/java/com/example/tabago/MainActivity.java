    ...
    private Handler noSmokingHandler = new Handler(new Handler.Callback() {
        ...
        public boolean handleMessage(Message msg) {
            if (msg.what == STOP_SMOKING) {
                // 현재 위치가 SmokingZone인지 확인
                boolean isSmokingZone = LocationUtility.isSmokingZone(MainActivity.this, SubActivity3.this.getLstMarkers());

                if (isSmokingZone) {
                    // 현재 위치가 SmokingZone이면 "count" 신호를 보냄
                    connectedThread.write("count");
                } else {
                    // 현재 위치가 SmokingZone이 아니면 "limit" 신호를 보냄
    ...
    public void increaseCountAndGraph() {
        ...
        runOnUiThread(new Runnable() {
            ...
            public void run() {
                ...
                countTextView.setText("현재 횟수: " + currentCount);
                // 일일 그래프 업데이트
                updateDailyChart();
            }
        });
